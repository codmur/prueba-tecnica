...
export const Episode = ({ trackName, releaseDate, trackTimeMillis }: any) => {
	const dur = dayjs.duration(trackTimeMillis);
	const horas = Math.floor(dur.asHours());
	const minutos = dur.minutes();
	return (
		<div className="flex flex-col gap-2 bg-white hover:cursor-pointer rounded-2xl shadow-sm px-4 py-3">
			...
			{/* <audio controls src={episode.previewUrl} className="w-full" /> */}
		</div>
	);
};
